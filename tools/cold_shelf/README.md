...
Planes (NO-FAN): **A** = one CF/GitHub tunnel (5 published surfaces / 2 family radii, not 5 shelves). **B** = alternate independent forge/archive tip-pack SLOT (`doi` null; Codeberg PASS at https://codeberg.org/AzielEliab/aziel-lockset-tip; archive.org PASS at https://archive.org/details/aziel-lockset-tip; Framagit awaiting tip-pack, url null). LIVE only after all three (`CNS-PLANE-B-ALL-TARGETS`). GitFlic refused (`CNS-GITFLIC-EMAIL`). GitLab extra (`CNS-GITLAB-CF-LOOP`). Zenodo tip-pack is refused (`CNS-ZENODO-IP-BAN`). **C** = USB airgap tarball + SHA256SUMS + verify script (primary; SLOT until `CNS-OPERATOR-ATTEST`). Extra **E/F/G** SLOTs = Launchpad, AfricArXiv/OSF, GitLab (`url` null; not required for Plane B LIVE). RESTORE-DRILL kept.
