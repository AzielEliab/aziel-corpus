# Zenodo tip-pack — refused (not Plane B working shelf)

Author: Aziel Eliab only
Spec: COLD-MULTI-SHELF-1.0
Umbrella: CROSS-NETWORK-SURVIVAL-1.0 · NO-LIE-NO-REWRITE-1.0
Lockset: AZLOCK-INGEST-REEXPAND-1.0
Published tip: `c831429befc221bd41caeb0a6d1c5361602db5684abab7af6d39714084b6b245`
cite.json / lockset `doi`: **null** (do not invent)

Zenodo is **not** the Plane B working shelf. Operator IP banned at Zenodo (`CNS-ZENODO-IP-BAN`). Status is **refused**. There is still no tip-pack DOI (`CNS-NO-TIP-DOI`). Do not invent one.

Plane B working path is an alternate independent forge/archive (Codeberg / archive.org / Framagit OR GitLab). GitFlic is optional later (`CNS-GITFLIC-EMAIL`) and does not block Plane B. See `ALT-FORGE-TIP-PACK-CHECKLIST.md`.

Paper deposits already cited in-repo (ShadowLock, DecisionGATE, TrajectoryLock, WhistleLock/FoldLock, EmployeeLock) stay paper deposits. `reuse_as_plane_b: false`. They are not tip-pack Plane B.

## Do not

- Invent a DOI to fill any shelf.
- Treat a paper record as the tip-pack.
- Claim Plane B is LIVE because a paper DOI resolves.
- Use Zenodo as the working Plane B upload path.

Refuse codes: `CNS-ZENODO-IP-BAN` · `CNS-NO-TIP-DOI` · `CNS-TIP-NOT-ON-DEPOSIT` · `CNS-NO-FAN-FAKE-DEPOSIT`
