...
def main():
    ...
    bf=sub.add_parser('backfill-review'); bf.add_argument('--limit',type=int,default=50); bf.add_argument('--force',action='store_true'); bf.add_argument('--record-id',default=''); bf.add_argument('--all',action='store_true',help='Walk every stored record (triad, zsolver, succession)')
    rc=sub.add_parser('recalibrate-all'); rc.add_argument('--force',action='store_true',help='Remint even already-V3 frozen scores'); rc.add_argument('--record-id',default='')
    ...
    elif a.cmd=='backfill-review': print(json.dumps(v.backfill_reviews(limit=a.limit,force=a.force,record_id=a.record_id or None,all_records=a.all),indent=2,default=str))
    elif a.cmd=='recalibrate-all': print(json.dumps(v.recalibrate_all(force=a.force,record_id=a.record_id or None),indent=2,default=str))
