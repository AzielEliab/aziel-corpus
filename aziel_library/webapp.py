...
def page(title,body):
    ...
    jeeves='''<button type="button" class="jeeves-fab" id="jeevesFab">Ask Jeeves</button><aside class="jeeves-drawer" id="jeevesDrawer" hidden><p><b>Ask Jeeves</b> — research assistant. Not sovereign. Not operator. Cannot change scores.</p><div class="jeeves-log" id="jeevesLog"></div><form id="jeevesAsk"><textarea name="q" rows="2" placeholder="Ask about a filed record…"></textarea><p><button>Ask</button></p></form></aside><script>(function(){var d=document.getElementById('jeevesDrawer'),l=document.getElementById('jeevesLog');document.getElementById('jeevesFab').addEventListener('click',function(){d.hidden=!d.hidden});document.getElementById('jeevesAsk').addEventListener('submit',function(e){e.preventDefault();var q=this.q.value.trim();if(!q)return;var p=document.createElement('p');p.textContent='You: '+q;l.appendChild(p);this.q.value='';fetch('/api/jeeves/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:q})}).then(r=>r.json()).then(j=>{var n=document.createElement('p');n.textContent='Jeeves: '+(j.answer||j.error||'');l.appendChild(n);});});})();</script>'''
