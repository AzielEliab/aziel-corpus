...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=='/intelligence':
            ...
            lib_upload=("<div class='card'><h3>Optional library upload</h3><p class='muted'>File a scan, transcript, or note through the same ingest as the shelf: structure, SPRE × CLCE × PhysLing, Bayesian, document hash-chain.</p><form method='post' action='/upload' enctype='multipart/form-data'><input type='file' name='files' multiple required> <button>Upload to library</button></form></div>" if self.mode=='master' else "")
            body=f"<div class='card'><h2>Aziel Intelligence Runtime</h2><p>Packages are local <b>.azm</b> models and <b>.azk</b> knowledge kits. Their manifests and payloads are hashed and verified.</p>{pkg_install}</div><div class='card'><h3>Optional local processors</h3><div class='grid'>{runtime_cards}</div>{proc_install}</div>{recovery}{lib_upload}<div class='card'><table><tr><th>ID</th><th>Kind</th><th>Type</th><th>Version</th><th>SHA-256</th><th>Status</th></tr>{rows}</table></div><div class='card'><h3>Native engines</h3><div class='grid'><div><b>AZIEL_TEXT_ENGINE</b><p class='muted'>Text, CSV/TSV, ZIP-office and conservative PDF text extraction.</p></div><div><b>AZIEL_HASH_VECTOR_V1</b><p class='muted'>512-dimensional deterministic local similarity vectors.</p></div><div><b>AZIEL_ENTITY_ENGINE</b><p class='muted'>Dates, people and AZK-gazetteer entities.</p></div><div><b>AZIEL_MODEL_RUNTIME</b><p class='muted'>Executes supported native AZM model families.</p></div></div></div>"
        ...
        if u.path.startswith('/record/'):
            ...
            zsolver_html=f"<div class='card'><h2>ZionPattern Solver</h2><div class='triad'><div class='metric'>{zdisp if zdisp is not None else '—'}</div><div><p>Secondary public score. Separate from the triad. Provisional and assistive. Does not solve cases. Hard cap 75% / uncertainty floor 25%.</p><p class='muted'>{html.escape(str(zsolver.get('disclaimer') or 'Provisional and assistive only. Author Aziel Eliab.'))}{zq}</p></div></div></div>"
            succ=r.get('succession') or {}
            schain=succ.get('chain') or []
            def _succ_links(rows, empty):
                if not rows: return f"<p class='muted'>{empty}</p>"
                return '<ul>'+''.join(f"<li><a href='/record/{html.escape(x['record_id'])}'>{html.escape(x.get('title') or x['record_id'])}</a></li>" for x in rows)+'</ul>'
            chain_html=' → '.join(f"<a href='/record/{html.escape(x['record_id'])}'>{html.escape(x.get('title') or x['record_id'])}</a>" for x in schain)
            succession_html=f"<div class='card'><h3>Succession</h3><p class='muted'>Exact-same-subject paper cites. Oldest to newest.</p><p><b>Supersedes</b></p>{_succ_links(succ.get('supersedes'),'Nothing earlier in this chain.')}<p><b>Superseded by</b></p>{_succ_links(succ.get('superseded_by'),'Nothing later in this chain.')}<p><b>Full chain</b></p><p>{chain_html}</p></div>" if len(schain)>=2 else ''
        ...
        if u.path in {'/download','/download/'} or (u.path.startswith('/v1/docs/') and u.path.rstrip('/').endswith('/download')):
            ...
            rid=(q.get('record') or q.get('record_id') or [''])[0]
            if u.path.startswith('/v1/docs/'):
                parts=[p for p in u.path.split('/') if p]
                if len(parts)>=4: hid=urllib.parse.unquote(parts[2])
            if hid:
                found=self.vault.find_record_id_by_hash(hid)
                if not found: return self.send_error(404,'hash not found')
                return self.redirect('/original/'+urllib.parse.quote(found,safe=''))
            if rid:
                return self.redirect('/original/'+urllib.parse.quote(rid,safe=''))
            return self.send_error(400,'record or hash required')
    ...
    def row(self,r):
        ...
        z=md.get('zsolver') or {}
        zdisp=z.get('display') if z.get('display') is not None else '—'
        return f"<tr><td><a href='/record/{r['record_id']}'><b>{html.escape(r['original_name'])}</b></a><br><span class='pill'>{html.escape(r['media_class'])}</span> <a class='button' href='/original/{urllib.parse.quote(r['record_id'],safe='')}'>Download</a></td><td><b>{display}</b> triad<br><b>{zdisp}</b> ZionPattern<br>{html.escape(r['primary_subject'])}<br>-> {html.escape(r['secondary_subject'])}</td><td>{snip}</td><td class='why'>{html.escape(r['classification_reason'])}</td><td class='hash'>{(html.escape(r['stored_path'])+'<br>') if self.mode=='master' else ''}{html.escape(r['sha256'][:20])}...</td></tr>"
