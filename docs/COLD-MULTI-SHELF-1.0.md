...
Plane A is one LIVE CF/GitHub tunnel (5 published surfaces / 2 family radii / 1 independent live). Plane B is an alt independent forge/archive SLOT until Codeberg + archive.org + GitFlic all hash-verify. Zenodo is refused (`CNS-ZENODO-IP-BAN`). Plane C is the USB airgap pack (SLOT until attested). Extra D/E/F/G tip-pack SLOTs stay url-null. Survival is bytes↔hash. Operator . Never publish . Identity: Aziel Eliab only.
