...
- **Softwares:** [https://www.azielcorpuslibrary.net/software](https://www.azielcorpuslibrary.net/software) — library Softwares catalog for aziel-runtime (Worker `GET /v1/software` is SSoT; `GET /runtime/v1/fraggate/list` is fallback only). Heading then list. No fixed product cap. Do not mash version + FragGate into Softwares copy. Tab count **42** (placements included). **41** live advisory engines. Isolation software_count **33**. Do not equate those counts. Ask Jeeves is FragGate op `jeeves` on aziel-corpus (suite help), `software_tab` false, and it is not a Softwares-tab card.
...
- **Aziel Runtime (this domain):** [https://www.azielcorpuslibrary.net/runtime](https://www.azielcorpuslibrary.net/runtime) — Aziel Runtime **2.0.0-rc1** (SoT LIVE `main` `231b02f` / version_id `a8f7fdc9`). FragGate is the single door (`fraggate_list` → `fraggate_describe` → `fraggate_call`). MCP Softwares via `fraggate_call` only. Prefer `/runtime/*`. Runtime `@id` [https://www.azieleliab.com/runtime#runtime](https://www.azieleliab.com/runtime#runtime). Mesh outlet [https://www.azielcorpuslibrary.net/v1/mesh/outlet](https://www.azielcorpuslibrary.net/v1/mesh/outlet) receives SoT fan-out (`docs/MESH-OUTLET-1.0.md`). Site inventory [https://www.azielcorpuslibrary.net/v1/inventory](https://www.azielcorpuslibrary.net/v1/inventory).
...
Every upload and download hashes and structurally verifies every file in the record or package, then re-scores with SPRE, a CLCE port (or live AZ-CLCE `/v1/score`), and **PhysLing Review (PLR)**. Suspected poison (official narrative without independent evidence, non-evidence advocacy, contradictory-only shells) is quarantined on the hash-chain — never silently deleted. A Bayesian posterior is stored as **unranked** metadata for manual peer-to-peer review and is never used to sort the shelf. A separate **HEURISTIC possibility** score asks whether the stated event could have occurred given lattice time×geo anchors. Adaptive learning appends LEARN / POISON_LEARN stamps to the hashchain lattice; recollection is tip + prev-hash verify. See `docs/HASHCHAIN-LATTICE-LEARN-1.0.md`. Verified ingest emits an AzielTether lattice tip. Suite Live Nodes (`GET /v1/mesh`, `GET /runtime/v1/mesh`) are human mesh presence + current website page viewers (`live_nodes`) — not Softwares (`software_nodes`) and not cited uses. Read-only QNM ON. Disable is refused. Those payloads cite **CROSS-NETWORK-SURVIVAL-1.0** (umbrella: if the network and live data die tomorrow, the chain still survives on cold copies — bytes↔hash; crawlers are extra shelves) over **MESH-SPLIT-WIRES-1.0** / **MESH-COLD-COPY-1.0** / die-with-pull / **MESH-REEXPAND-1.0** / **MESH-REHEAL-1.0**, plus executable **COLD-MULTI-SHELF-1.0** (`GET /shelves`, `tools/cold_shelf/`) and **QNS-CD-1.0** (photon QNS1 packet transfer) as a hub / Worker mesh cross-map only — local `qnsd` is coded in [qnm-node](https://github.com/AzielEliab/qnm-node); runtime cites + catalog field live in [aziel-runtime](https://github.com/AzielEliab/aziel-runtime); [AZInterface](https://github.com/AzielEliab/azinterface) has pair custody. No public `qnsd` proxy. No Node Gate. Human help: `/help.txt` · `/addendum.txt` · `/help/how-to-read-scores.txt` · `/help/how-to-cite.txt`. See `REVIEW.md` and `docs/CROSS-NETWORK-SURVIVAL-1.0.md`.
