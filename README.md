...
- **Donate:** [https://www.azielcorpuslibrary.net/donate](https://www.azielcorpuslibrary.net/donate) — static tab + chrome strip (RL-WP-0.1-library). Does not touch KV. Addresses are pasted at publish.
- **Packed index / tunnel standby:** `GET /v1/search` filters one packed key (`library:index:v1`). `GET /v1/library-index` and `GET /v1/health` (`role=standby`). Design notes: `docs/TUN-WP-0.1.md`, `docs/RL-WP-0.1-library.md`. Worker is standby catalog of truth; cloudflared may be primary later. Not a VPN. Library-scope only — not aziel-runtime, not godlock.uk.
