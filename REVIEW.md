...
After deploy, trigger `GET /v1/verify-backfill?rebuild=1` so existing D1 scores land on the packed shelf and tips. The rebuild is cursor-paginated (default 25 rows, ~4s budget) so one request returns JSON promptly with `done` + `next_cursor`. Packed `library:index:v1` + homepage HTML cache refresh is deferred to `ctx.waitUntil` and must not block the response. Repeat with `?cursor=` or `?all=1` until `done:true`. `/v1/verify-backfill` does not start the per-request background backfill/geo walk (that walk was racing D1 with rebuild). `?status=1` is the light progress read.
