...
After deploy, trigger `GET /v1/verify-backfill?rebuild=1` so existing D1 scores land on the packed shelf and tips. The rebuild is cursor-paginated (default 25 rows) so one request finishes inside Worker limits and returns `done` + `next_cursor`. Repeat with `?cursor=` or `?all=1` until `done:true`. Each chunk also refreshes packed `library:index:v1` from stored scores and busts homepage HTML cache, so already-scored docs drop “pending backfill” without waiting for every tip. `done:true` on the scoring walk continues that chunked shelf sync; it no longer tries to reconcile the whole table in one request.
