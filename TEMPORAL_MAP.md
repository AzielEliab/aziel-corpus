...
Successful ingest also runs the **upload→pin** pipeline: extract anchors, pin, append `MAP_PIN` + `LEARN` + `POSSIBILITY_SCORE` on the document hashchain (`GET /v1/pin?record_id=`). Fail closed if structure or poison blocks. Possibility is a HEURISTIC derived view over those lattice pins (`GET /v1/possibility?record_id=`). Sister inspection frame: [4DMap 4DM-WP-1.0](https://github.com/AzielEliab/4dmap) — not a live ICANN mesh DNS. Adaptive learning via hashchain lattice for recollection and reasoning.
