...
import { appendLedger, ensureLedger, hashPayload } from "./ledger.js";
...
export async function persistReview(env, { recordId, library, sha256, title, createdBy, event, structure, review }) {
  ...
  const receipt = await appendLedger(env, "STRUCTURE_VERIFY", {
    record_id: recordId,
    library,
    sha256,
    event,
  ...
  await appendLedger(env, "REVIEW_SCORE", {
    record_id: recordId,
    library,
    sha256,
    event,
  ...
  if (qStatus !== "CLEAR") {
    await appendLedger(env, "POISON_QUARANTINE", {
      record_id: recordId,
      library,
      sha256,
      status: qStatus,
  ...
  if (structure && structure.ok) {
    ...
    tipEntry = await appendLedger(env, "LATTICE_ANCHOR", {
      record_id: recordId,
      library,
      sha256,
      schema: tip.schema,
...
export async function addPeerReview(env, { recordId, stance, body, signed }) {
  ...
  const entry = await appendLedger(env, "PEER_REVIEW", {
    record_id: recordId,
    library: row.library,
