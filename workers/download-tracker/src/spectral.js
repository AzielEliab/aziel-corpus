/**
 * SpectralLock overlay assist for hosted OCR. Author: Aziel Eliab.
 *
 * Ported from SpectralLock's Worker overlay (advisory, not a spectrometer).
 * OCR uses a higher side cap than the 256px hosted preview so the enhanced
...
export const PURE = ["zero", "tazel", "vyrn", "uv"];
export const COMPOSITE = ["rosetta", "zen", "chaos", "balance"];
export const LIVE = PURE.concat(COMPOSITE);

export const MODES = [
  { id: "zero", paper: "ZSA-1.0", status: "live", hex: "#6F6485", hue: 260, summary: "Equilibrium / geometry (ZSA-1.0)." },
  { id: "tazel", paper: "TSA-1.0", status: "live", hex: "#1EC9A5", hue: 170, summary: "Hidden ink / revelation (TSA-1.0)." },
  { id: "vyrn", paper: "VSA-1.0", status: "live", hex: "#C00066", hue: 350, summary: "Pressure / purification (VSA-1.0)." },
  { id: "uv", paper: "UVSA-1.0", status: "live", hex: null, hue: null, summary: "Synthetic 365–400 nm look. Not a real UV lamp." },
  { id: "rosetta", paper: "RSA-2.0", status: "live", hex: null, hue: null, summary: "0.40·Z′ + 0.35·T′ + 0.25·V′ after normalize." },
  { id: "zen", paper: "ZENA-1.0", status: "live", hex: null, hue: null, summary: "(Z′ + T′ + U′ + V′) / 4 after normalize." },
  { id: "chaos", paper: "CSA-1.0", status: "live", hex: null, hue: null, summary: "0.40·U′ + 0.35·V′ + 0.20·T′ + 0.05·Z′ after normalize." },
  { id: "balance", paper: "BSA", status: "live", hex: null, hue: null, summary: "α·Zen + (1−α)·Chaos. Never invents marks." },
];
...
export function normalizeLenses(raw) {
  ...
  for (const item of list) {
    ...
    if (LIVE.includes(id) && !out.includes(id)) out.push(id);
  }
  return out;
}

function rosettaReweight(pures) {
  const weights = {};
  let sum = 0;
  for (const id of pures) {
    const w = ROSETTA_W[id];
    if (!(w > 0)) return null;
    weights[id] = w;
    sum += w;
  }
  if (sum <= EPS) return null;
  for (const id of pures) weights[id] /= sum;
  return weights;
}

export function resolveLensPlan(selected) {
  const ids = normalizeLenses(selected);
  if (!ids.length) return { kind: "none", ids: [], mode: null, weights: null, composites: [] };
  if (ids.length === 1) {
    return { kind: "single", ids, mode: ids[0], weights: null, composites: ids[0] && COMPOSITE.includes(ids[0]) ? [ids[0]] : [] };
  }
  const pures = ids.filter((id) => PURE.includes(id));
  const composites = ids.filter((id) => COMPOSITE.includes(id));
  if (composites.length) {
    return { kind: "composites", ids, mode: null, weights: null, composites, pures };
  }
  const reweighted = rosettaReweight(pures);
  if (reweighted) return { kind: "weights", ids, mode: null, weights: reweighted, composites: [] };
  const eq = pures.length ? 1 / pures.length : 0;
  const weights = {};
  for (const id of pures) weights[id] = eq;
  return { kind: "weights", ids, mode: null, weights, composites: [] };
}
...
function toLuma(buf, w, h) {
  ...
  for (let i = 0, p = 0; i < out.length; i++, p += 3) out[i] = luma(buf[p], buf[p + 1], buf[p + 2]);
  return out;
}
function norm01(arr) {
...
function modeVyrn(buf) {
  ...
  for (let p = 0; p < buf.length; p += 3) {
    ...
    out[p + 1] = clamp01(ng * (1 - 0.22 * w) * (1 - 0.25 * cyan));
...
function baseChannels(buf, w, h) {
  return {
    zero: norm01(toLuma(modeZero(buf), w, h)),
    tazel: norm01(toLuma(modeTazel(buf), w, h)),
    vyrn: norm01(toLuma(modeVyrn(buf), w, h)),
    uv: norm01(toLuma(modeUv(buf), w, h)),
  };
}
function applyNamedMode(buf, w, h, mode) {
...
function averageRgb(buffers) {
  if (!buffers.length) return null;
  const out = new Float32Array(buffers[0].length);
...
export function applyPlan(buf, w, h, plan) {
  if (!plan || plan.kind === "none") return buf;
  if (plan.kind === "single") return applyNamedMode(buf, w, h, plan.mode);
  if (plan.kind === "weights") {
    const ch = baseChannels(buf, w, h);
    return grayToRgb(mixLuma(ch, plan.weights || {}));
  }
  if (plan.kind === "composites") {
    const parts = (plan.composites || []).map((mode) => applyNamedMode(buf, w, h, mode));
    return averageRgb(parts) || buf;
  }
  return buf;
}
...
function capSide(buf, w, h, maxSide) {
  const side = Math.max(w, h);
  if (side <= maxSide) return { buf, w, h };
...
export async function enhancePngBytes(bytes, selected, opts = {}) {
  const plan = resolveLensPlan(selected);
  if (plan.kind === "none") return { ok: false, skipped: true, plan };
  let decoded;
  try {
    decoded = await decodePng(bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes));
  } catch (err) {
    return { ok: false, error: String(err && err.message ? err.message : err), plan, advisory: LIMITATION };
  }
  const maxSide = Number(opts.maxSide) > 0 ? Number(opts.maxSide) : OCR_MAX_SIDE;
  const capped = capSide(decoded.buf, decoded.w, decoded.h, maxSide);
  const out = applyPlan(capped.buf, capped.w, capped.h, plan);
  ...
  return {
    ...
    processor: "SPECTRALLOCK_OCR_ASSIST",
    processor_version: VERSION,
    advisory: LIMITATION,
  };
}

export function paperFor(id) {
