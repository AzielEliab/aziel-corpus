...
test("visible ingest-receipt chrome is off; machine tip metas stay", () => {
  assert.equal(ingestReceiptStrip(), "");
  assert.doesNotMatch(ingestReceiptStrip(), /az-ingest-receipt/);
  assert.doesNotMatch(ingestReceiptStrip(), /1 Chronicles 15:20/);
  assert.doesNotMatch(ingestReceiptStrip(), BANNED);
  const head = ingestReceiptHead();
  assert.match(head, /name="aziel-lockset-id"/);
  assert.match(head, /name="aziel-lockset-tip"/);
  assert.match(head, /name="aziel-cite-rule"/);
  assert.match(head, /name="aziel-survive-rule"/);
  assert.match(head, /name="aziel-cross-network-survival"/);
  assert.match(head, /name="aziel-cross-network-survival-rule"/);
  assert.match(head, /name="aziel-no-lie-no-rewrite"/);
  assert.match(head, /name="aziel-no-lie-no-rewrite-rule"/);
  assert.match(head, new RegExp(LOCKSET_TIP));
  assert.match(head, /AZLOCK-INGEST-REEXPAND-1\.0/);
  assert.match(head, /cite, don't merge/);
  assert.match(head, /href="\/lockset\.json"/);
  const home = page("Corpus Search", homeBody({ rows: [], host: "https://www.azielcorpuslibrary.net" }), { path: "/", kind: "search" });
  assert.match(home, /name="aziel-lockset-id"/);
  assert.match(home, /name="aziel-lockset-tip"/);
  ...
  assert.doesNotMatch(home, /id="az-ingest-receipt"/);
  assert.doesNotMatch(home, /class="ingest-receipt"/);
  assert.doesNotMatch(home, /<aside class="ingest-receipt"/);
  assert.doesNotMatch(home, BOTH_MUSICIANS_P);
  const homeBodyHtml = home.split("</head>")[1] || "";
  assert.doesNotMatch(homeBodyHtml, /AZLOCK-INGEST-REEXPAND-1\.0/);
  assert.doesNotMatch(homeBodyHtml, /cite, don't merge/);
  assert.doesNotMatch(homeBodyHtml, /CROSS-NETWORK-SURVIVAL/);
  assert.doesNotMatch(homeBodyHtml, /NO-LIE \/ NO-REWRITE/);
  const other = page("Receipts", "<div class=\"card\">ledger</div>", { path: "/receipts", kind: "article" });
  assert.match(other, /name="aziel-lockset-tip"/);
  assert.doesNotMatch(other, /id="az-ingest-receipt"/);
  assert.doesNotMatch(other.split("</head>")[1] || "", /AZLOCK-INGEST-REEXPAND-1\.0/);
