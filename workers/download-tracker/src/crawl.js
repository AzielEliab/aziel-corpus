...
const PRODUCT_LINES = [
  ...
  ["Suite mesh / Live Nodes (mesh presence + page viewers; not Softwares; not uses)", HOST + "/v1/mesh", HOST + "/runtime/v1/mesh"],
...
export function llmsDoc(limitation, survival) {
  return "# Aziel Digital Library v2.7.0\n\n"
    ...
    + "Live Nodes (mesh presence + current website page viewers; not Softwares; not uses): " + HOST + "/v1/mesh\n"
    ...
    + "The Software hub mirrors the live runtime catalog. Cards grow with GET /v1/software (fallback fraggate/list). PeaceLock, AZMail, AZBrowser, and later slugs appear automatically. No hard-coded 27 cap. Door extras AZNet and FragGate (separate app Workers) and EmbryoLock are listed without dropping catalog engines. AZCoherence (azcoherence) is a Softwares extra / peer-map fallback (Plain, scoring-review) so cite surfaces stay mapped if the live catalog is thin. FoldLock (foldlock) is a Softwares Language extra / cold-shelf SLOT hook; never fold the lockset tip. "
    + SPECTRALLOCK_NOTE + " "
    + PEACELOCK_NOTE + " "
    + "Trades-Runtime (trades-runtime) is a public Softwares extra / BYO field OS; live_backends false. "
    + WHITESTONE_NOTE + " "
    + ARK_NOTE + " "
    + "One public version from Softwares SSoT: " + RUNTIME_VERSION + " (GET /v1/software catalog.version). Product versions stay on Softwares cards.\n"
