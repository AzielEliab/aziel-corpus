...
- Cap-7 bridge cite (Plane A): \`GET ${HOST}/bridge.json\`
- First-class websites: azcorpus (Corpus / Lamb Lens, ${HOST}/corpus) and azlibrary (royal-purple Aziel Library, ${HOST}/aziel-library). Anyone may download both.
- Products index: \`GET ${HOST}/v1/products\`
- Design packs (mesh nodes; names are not hub DNS): \`GET ${HOST}/v1/design-pack\` · counted \`GET ${HOST}/download?product=azcorpus|azlibrary\`
...
function openapi() {
  return {
    ...
    paths: {
      ...
      "/v1/ingest": { post: { summary: "AI/OpenAPI upload (ingest/receipt). Session cookie writes Corpus (Lamb Lens). X-Aziel-Operator-Token writes live hub azlibrary only. Anonymous JSON refused. Cap-7 mesh names are designs — this op does not write azcorpus/azlibrary mesh sites. CROSS-NETWORK-SURVIVAL on the receipt. Compatible AI clients: " + AI_CLIENTS + ".", operationId: "ingestRecord" } },
      "/v1/jeeves/upload": { post: { summary: "Ask Jeeves Add — same ingest as the shelf (structure, SPRE × CLCE × PhysLing, Bayesian). Signed-in public writes Corpus; operator writes live hub Aziel Library. Not a Cap-7 mesh write.", operationId: "jeevesUpload" } },
      "/v1/operator/library-ingest": { post: { summary: "Operator Aziel Library ingest (SOFTWARE-SITE-DOSSIER-1.0). Same ingestRecord pipeline as Jeeves/shelf. Requires X-Aziel-Operator-Token or operator session. Writes live hub azlibrary only — not a Cap-7 mesh name. Idempotent same-SHA; exact-same-subject supersedes. Writes discovery metadata sidecar in the same ingest. No public write hole.", operationId: "operatorLibraryIngest" } },
      "/v1/products": { get: { summary: "First-class website products: azcorpus (Corpus / Lamb Lens) and azlibrary (royal-purple Aziel Library). Anyone may download both. azlibrary upload uses X-Aziel-Operator-Token on the live hub only.", operationId: "websiteProducts" } },
      "/v1/design-pack": { get: { summary: "Index of Cap-7 design+content packs. First-class websites: azcorpus, azlibrary. Sister-hub designs are cite-only. Names do not resolve to Plane A hubs.", operationId: "designPackIndex" } },
      "/v1/design-pack/{slug}": { get: { summary: "One Cap-7 website design+content pack (azcorpus, azlibrary, …). Includes mesh_pull hash-verify steps. Pull-only cold copy. Not live public DNS. CROSS-NETWORK-SURVIVAL.", operationId: "designPack", parameters: [{ name: "slug", in: "path", required: true, schema: { type: "string" } }] } },
      "/v1/design-pack/{slug}/download": { get: { summary: "Attachment of one design+content pack. Does not increment. Counted twin: GET /download?product=azcorpus|azlibrary. Anyone may download.", operationId: "designPackDownload", parameters: [{ name: "slug", in: "path", required: true, schema: { type: "string" } }] } },
      "/bridge.json": { get: { summary: "Plane A semantic bridge cite for MirageGrid Cap-7. Mesh names are NOT ICANN. Plane A hubs stay themselves. No AZ-GEN publish cadence. Future MirageGrid /bridge.json is cited, not claimed live.", operationId: "bridgeJson" } },
