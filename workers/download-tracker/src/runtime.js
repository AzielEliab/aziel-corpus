...
- Design packs (mesh nodes; names are not hub DNS): \`GET ${HOST}/v1/design-pack\`
...
- \`POST /v1/operator/library-ingest\` (operator token or session only; live hub Aziel Library; SOFTWARE-SITE-DOSSIER-1.0)
...
function openapi() {
  return {
    ...
    paths: {
      ...
      "/v1/design-pack": { get: { summary: "Index of Cap-7 design+content packs for download-to-mesh-nodes. azcorpus/azlibrary/sister hubs are designs; they do not resolve to Plane A hubs.", operationId: "designPackIndex" } },
      "/v1/design-pack/{slug}": { get: { summary: "One Cap-7 design+content pack (azcorpus, azlibrary, azeliab, godlock, hedidntjump). Pull-only cold copy. Not live public DNS. CROSS-NETWORK-SURVIVAL.", operationId: "designPack", parameters: [{ name: "slug", in: "path", required: true, schema: { type: "string" } }] } },
      ...
      "/download": { get: { summary: "Counted Softwares zip (asset=), counted record download (record=AZDOC-…), or counted content-hash download (hash=SHA-256). Streams the hosted zip. Honest 200 JSON if the asset is missing. HTTP 200, no silent 302. AI clients: aziel-corpus_download.", operationId: "download", parameters: [{ name: "asset", in: "query", schema: { type: "string" } }, { name: "record", in: "query", schema: { type: "string" } }, { name: "hash", in: "query", schema: { type: "string" } }, { name: "sha256", in: "query", schema: { type: "string" } }] } },
