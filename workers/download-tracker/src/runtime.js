...
function openapi() {
  return {
    ...
    paths: {
      ...
      "/v1/mesh": { get: { summary: "Suite mesh status. Live Nodes (live_nodes) = human mesh users + cited human uses. software_nodes is Softwares and never Live Nodes. Read-only QNM ON. Counts/status rollup. Proxies /v1/mesh. Cites QNS-CD-1.0, CROSS-NETWORK-SURVIVAL-1.0, MESH-SPLIT-WIRES-1.0, MESH-COLD-COPY-1.0, MESH-REEXPAND-1.0, MESH-REHEAL-1.0, NO-LIE-NO-REWRITE-1.0. Author Aziel Eliab.", operationId: "mesh" } },
      "/v1/mesh/status": { get: { summary: "Suite mesh status alias. Live Nodes = human mesh users + cited human uses, not software_nodes. Read-only QNM ON. Author Aziel Eliab.", operationId: "meshStatus" } },
      "/v1/mesh/nodes": { get: { summary: "Suite mesh roster. Live Nodes · N is human mesh users + cited human uses from live_nodes (not software_nodes). Author Aziel Eliab.", operationId: "meshNodes" } },
  ...
  if (path === "/v1" || path === "/v1/health") {
    ...
    const res = json({
      ...
      review: {
        ...
        mesh: "GET /v1/mesh and /runtime/v1/mesh — suite node mesh; Live Nodes = human mesh users + cited human uses (not software_nodes). Read-only QNM ON. Counts/status rollup. Disable refused. QNS-CD-1.0 + CROSS-NETWORK-SURVIVAL-1.0 + MESH-SPLIT-WIRES-1.0 + MESH-COLD-COPY-1.0 + MESH-REEXPAND-1.0 + MESH-REHEAL-1.0 + NO-LIE-NO-REWRITE-1.0. Survival is bytes↔hash. Live body sync refused. Re-expand is archive restore. Reheal is self tip + trusted pull or phoenix-WAIT. Network never lies to stay alive. No rewrite key. Author Aziel Eliab.",
