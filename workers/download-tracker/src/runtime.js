...
- Suite mesh (read-only QNM ON): \`GET ${HOST}/v1/mesh\` · \`GET ${HOST}/runtime/v1/mesh\` (origin ${CATALOG}/v1/mesh). Live Nodes cite **QNS-CD-1.0** (photon QNS1 packet transfer) as a hub / Worker mesh cross-map. Public rollup is counts/status — not the cell. Split-wires + cold-copy survival (vault-on-transfer multiplies cold copies; live body sync refused). Local \`qnsd\` is coded in https://github.com/AzielEliab/qnm-node. Runtime cites + catalog field live in https://github.com/AzielEliab/aziel-runtime. AZInterface has pair custody. Not a Softwares-tab product. No public \`qnsd\` proxy. No Node Gate. Disable is refused.
...
- \`GET /v1/mesh\` · \`GET /v1/mesh/status\` · \`GET /v1/mesh/nodes\` (suite mesh; read-only QNM ON; counts/status rollup, not the cell; QNS-CD-1.0 + MESH-SPLIT-WIRES-1.0 + MESH-COLD-COPY-1.0 on the payload)
...
function openapi() {
  return {
    ...
    paths: {
      ...
      "/v1/mesh": { get: { summary: "Suite decentralized node mesh status. Read-only QNM ON. Counts/status rollup — not the cell. Proxies /v1/mesh. Cites QNS-CD-1.0, MESH-SPLIT-WIRES-1.0, MESH-COLD-COPY-1.0. Author Aziel Eliab.", operationId: "mesh" } },
      "/v1/mesh/status": { get: { summary: "Suite mesh status alias. Read-only QNM ON. Counts/status — not the cell. QNS-CD-1.0 + split-wires + cold-copy. Author Aziel Eliab.", operationId: "meshStatus" } },
  ...
  if (path === "/v1" || path === "/v1/health") {
    ...
    const res = json({
      ...
      review: {
        ...
        mesh: "GET /v1/mesh and /runtime/v1/mesh — suite node mesh; read-only QNM ON. Counts/status rollup — not the cell. Disable refused. QNS-CD-1.0 + MESH-SPLIT-WIRES-1.0 + MESH-COLD-COPY-1.0. Live body sync refused. Author Aziel Eliab.",
