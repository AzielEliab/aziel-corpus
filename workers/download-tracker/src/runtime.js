...
- \`GET /v1/mesh\` · \`GET /v1/mesh/status\` · \`GET /v1/mesh/nodes\` (suite mesh; Live Nodes = human mesh presence + current page viewers, not software_nodes, not uses; read-only QNM ON; counts/status rollup; QNS-CD-1.0 + CROSS-NETWORK-SURVIVAL-1.0 + MESH-SPLIT-WIRES-1.0 + MESH-COLD-COPY-1.0 + MESH-REEXPAND-1.0 + MESH-REHEAL-1.0 + NO-LIE-NO-REWRITE-1.0 on the payload)
- \`GET /v1/presence\` · \`POST /v1/presence\` (local page-viewer heartbeat; bots do not count; does not increment views)
...
function openapi() {
  return {
    ...
    paths: {
      ...
      "/v1/mesh/site-presence": { post: { summary: "Hub report of concurrent human page viewers to runtime SSoT (aziel-runtime#154). kind=human-page. Bots refused locally. Does not increment views or USES. Author Aziel Eliab.", operationId: "meshSitePresence" } },
      ...
      "/runtime/v1/mesh/nodes": { get: { summary: "Same-origin proxy of aziel-runtime /v1/mesh/nodes. Live Nodes · N is mesh presence + page viewers, not Softwares, not uses.", operationId: "runtimeProxyMeshNodes" } },
...
export async function handleRuntimeApi(request, url, env, ctx) {
  ...
  if (path === "/v1" || path === "/v1/health") {
    ...
    const res = json({
      ...
      review: {
        ...
        mesh: "GET /v1/mesh and /runtime/v1/mesh — suite node mesh; Live Nodes = human mesh presence + current azielcorpuslibrary.net page viewers (not software_nodes, not uses). Nodes = users + uses. Prefer runtime /v1/mesh once it aggregates; no double-count; no bot inflation. Read-only QNM ON. Author Aziel Eliab.",
  ...
  if ((path === "/v1/recalibrate-all" || (path === "/v1/verify-backfill" && (url.searchParams.get("recalibrate") === "1" || url.searchParams.get("recalibrate") === "true"))) && request.method === "GET") {
    const force = url.searchParams.get("force") === "1" || url.searchParams.get("force") === "true";
    const all = url.searchParams.get("all") === "1" || url.searchParams.get("all") === "true";
    const statusOnly = url.searchParams.get("status") === "1" || url.searchParams.get("status") === "true";
    const recordId = (url.searchParams.get("record_id") || url.searchParams.get("id") || "").trim() || null;
    if (statusOnly) return json({ ...(await recalibrateAllStatus(env)), limitation: LIMITATION });
    if (recordId) {
