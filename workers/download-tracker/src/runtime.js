...
- Suite mesh (read-only QNM ON): \`GET ${HOST}/v1/mesh\` · \`GET ${HOST}/runtime/v1/mesh\` (origin ${CATALOG}/v1/mesh). Live Nodes cite **QNS-CD-1.0** (photon QNS1 packet transfer) as a hub / Worker mesh cross-map. Local \`qnsd\` is coded in https://github.com/AzielEliab/qnm-node. Runtime cites + catalog field live in https://github.com/AzielEliab/aziel-runtime. AZInterface has pair custody. Not a Softwares-tab product. No public \`qnsd\` proxy. No Node Gate. Disable is refused.
...
- \`GET /v1/mesh\` · \`GET /v1/mesh/status\` · \`GET /v1/mesh/nodes\` (suite mesh; read-only QNM ON; QNS-CD-1.0 cross-map on the payload)
...
function openapi() {
  return {
    ...
    paths: {
      ...
      "/v1/mesh": { get: { summary: "Suite decentralized node mesh status. Read-only QNM ON. Proxies /v1/mesh. Cites QNS-CD-1.0 (photon QNS1; hub cite only; no public qnsd). Author Aziel Eliab.", operationId: "mesh" } },
      "/v1/mesh/status": { get: { summary: "Suite mesh status alias. Read-only QNM ON. QNS-CD-1.0 cross-map. Author Aziel Eliab.", operationId: "meshStatus" } },
      "/v1/mesh/nodes": { get: { summary: "Live Nodes list for the suite mesh. Live Nodes · N while mesh on. QNS-CD-1.0 cross-map. Author Aziel Eliab.", operationId: "meshNodes" } },
      ...
      "/AzielEliab": { get: { summary: "Aziel Eliab — author profile page (HTML). Visible 1 Chronicles 15:20 lock under H1. Corresponds with https://godlock.uk/AzielEliab. Legacy /about and /aboutme permanently redirect here.", operationId: "azielEliab" } },
      "/who": { get: { summary: "Who is Aziel Eliab (HTML). H1 lock names both Levitical musicians in 1 Chronicles 15:20. Person @id https://www.azieleliab.com/#aziel.", operationId: "whoHtml" } },
      ...
      "/who-is": { get: { summary: "Plain-text alias of /who-is-aziel-eliab.txt. Prefer HTML /who for the visible 15:20 lock.", operationId: "whoIs" } },
  ...
  if (path === "/v1" || path === "/v1/health") {
    ...
    const res = json({
      ...
      review: {
        ...
        mesh: "GET /v1/mesh and /runtime/v1/mesh — suite node mesh; read-only QNM ON. Disable refused. QNS-CD-1.0 photon QNS1 cross-map (hub cite only; local qnsd in qnm-node; no public proxy; no Node Gate). Author Aziel Eliab.",
