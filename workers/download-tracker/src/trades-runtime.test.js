...
test("Trades-Runtime cite is a local-first Softwares extra, not a FragGate engine", () => {
  ...
  assert.match(TRADES_RUNTIME.note, /Not a FragGate-exec true engine/);
  assert.match(TRADES_RUNTIME.note, /Not aziel-runtime wholesale/);
...
test("cite.json / llms.txt / humans.txt / ai.txt cite Trades-Runtime honestly", () => {
  ...
  assert.match(cite.trades_runtime.how_to_cite, /Trades-Runtime 0\.3\.3/);
  ...
  assert.match(llms, /not a FragGate-exec true engine/i);
