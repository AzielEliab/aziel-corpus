...
test("Trades-Runtime cite is a local-first Softwares extra, not a FragGate engine", () => {
  ...
  assert.equal(TRADES_RUNTIME_VERSION, "0.4.6");
  assert.equal(TRADES_RUNTIME.version, TRADES_RUNTIME_VERSION);
  assert.equal(TRADES_RUNTIME_SOFTWARE_EXTRA.version, "0.4.6");
  ...
  assert.match(TRADES_RUNTIME.how_to_cite, /Trades-Runtime 0\.4\.6/);
...
test("cite.json / llms.txt / humans.txt / ai.txt cite Trades-Runtime honestly", () => {
  ...
  assert.match(cite.trades_runtime.how_to_cite, /Trades-Runtime 0\.4\.6/);
  assert.equal(cite.trades_runtime.version, "0.4.6");
  assert.equal(cite.products_trades_runtime.version, "0.4.6");
