...
export function defaultDescription(kind, runtimeVersion) {
  ...
  if (kind === "forensics" || kind === "intelligence") return "Hosted forensics, OCR, SpectralLock lenses, and Whisper transcription for Aziel Digital Library. SpectralLock leftover-bytes recover is honest; opaque empty refuses SL-UNREDACT-OPAQUE; corpus OCR does not unredact by guessing. Author Aziel Eliab.";
  if (kind === "ocr") return "Hosted OCR and advisory SpectralLock lenses for Aziel Digital Library. Leftover container bytes recover honestly; opaque rewrite refuses SL-UNREDACT-OPAQUE; heatmap ≠ transcript; corpus OCR does not unredact by guessing. Author Aziel Eliab.";
