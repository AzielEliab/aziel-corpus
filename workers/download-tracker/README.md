...
- GET /sitemap-records.xml: every packed-index record — HTML + metadata.json + `.json` alias + `/llms.txt` + `/cite.json` (linked from sitemap-index.xml). One KV get of `library:index:v1`, union the D1 shelf. No `KV.list()`.
...
- GET /v1/library-index: packed shelf cards (`library:index:v1`). One KV get + Cache-Control. No PDF bodies. Cards include `metadata_url`, `llms_url`, and `cite_url`.
...
- GET/POST /v1/mesh/outlet  MESH-OUTLET-1.0 cite-sync consumer. Public GET returns the last-known runtime SoT (version, git, Softwares tab count 42 vs 41 live engines vs isolation software_count 33, Ask Jeeves suite help). POST and `?sync=1` require `X-Aziel-Operator-Token`. Does not increment downloads. Does not invent library rows. See `docs/MESH-OUTLET-1.0.md`.
- GET /v1/inventory  peer-hub library inventory. Unreachable hubs keep last-known rows and say unreachable. `?sync=1` requires the operator token. No invented AZDOC rows.
- GET /v1/mesh and /runtime/v1/mesh  suite node mesh status (read-only QNM ON; Live Nodes · N = human mesh presence + current azielcorpuslibrary.net page viewers, never software_nodes, never uses). Nodes · N = human mesh users + cited uses. Prefer runtime `/v1/mesh` once it aggregates; no double-count; no bot inflation. GET /v1/presence is the local heartbeat. Payload cites CROSS-NETWORK-SURVIVAL-1.0 (umbrella: bytes↔hash on independent shelves; crawlers are extra shelves) over MESH-SPLIT-WIRES-1.0 / MESH-COLD-COPY-1.0 / die-with-pull / MESH-REEXPAND-1.0 / MESH-REHEAL-1.0, plus NO-LIE-NO-REWRITE-1.0 (network never lies to stay alive; no rewrite key; cites live lockset AZLOCK-INGEST-REEXPAND-1.0) and QNS-CD-1.0 (photon QNS1 packet transfer) as a hub / Worker mesh cross-map: local qnsd in https://github.com/AzielEliab/qnm-node, runtime cites in https://github.com/AzielEliab/aziel-runtime, pair custody in AZInterface. No public qnsd proxy. No Node Gate. Disable is refused. GET /v1/mesh never enables radios.
...
- GET /v1/recalibrate-all  TRIAD V3 remint of stored papers (own cursor). Single-walker lock TTL 90s; concurrent walk returns `RECALIBRATE_LOCKED` and does not advance the cursor. Repeat `?all=1` until `done:true`. `?status=1` progress (cumulative scored/skipped + lock). `?force=1` remints even already-V3. Alias: `GET /v1/verify-backfill?recalibrate=1`. Cron defers while the lock is held. Local: `python3 tools/recalibrate_all.py`.
